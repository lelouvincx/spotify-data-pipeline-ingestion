...
def load_data_to_redshift(context, upstream):
    if upstream is None:
        return None

    # load data to target
    context.log.info("Load data to Redshift")
    ...
    # Execute
    db_loader = RedshiftLoader(upstream.get("target_db_params"))
    result = db_loader.load_data(pd_stag, upstream)
    context.log.info(f"Batch inserted status: {result}")
    return result


def factory_for_load_data_to_redshift(name):
    @op(name=name)
    def _load_data_to_redshift(context, upstream):
        return load_data_to_redshift(context, upstream)

    return _load_data_to_redshift
